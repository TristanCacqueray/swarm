...
stepCEK cek = case cek of
  ...
  In (TSPair t1 t2) e k              -> return $ In t1 e (FSnd t2 e : k)
  ...
  In tm@(TSDef x _ t) e k -> withExceptions k $ do
  ...
  In (TSBind mx t1 t2) e k           -> return $ Out (VBind mx t1 t2 e) k

  -- Delay expressions immediately turn into VDelay values, awaiting
  -- application of 'Force'.
  In (TSDelay t) e k                  -> return $ Out (VDelay Nothing t e) k
  ...
  Up  exn []                            -> return $ In (TApp (noloc $ TConst Say) (noloc $ TString (formatExn exn))) empty [FExec]
